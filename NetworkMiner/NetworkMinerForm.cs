        ...
        */
            ...
            if (PacketParser.Utils.SystemHelper.IsRunningOnMono())
            {
                ...
                try
                {
                    ...
                    System.Net.ServicePointManager.ServerCertificateValidationCallback = new System.Net.Security.RemoteCertificateValidationCallback(
                        ...
                        {
                            PacketParser.Utils.Logger.Log("Performing custom SSL validation for Mono", System.Diagnostics.EventLogEntryType.Information);
                            ...
                            {
                                PacketParser.Utils.Logger.Log("SSL validation error: " + policyErrors.ToString(), System.Diagnostics.EventLogEntryType.Warning);
                                return false;
                            }
                            ...
                            else if (policyErrors == System.Net.Security.SslPolicyErrors.RemoteCertificateChainErrors)
                            {
                                ...
                                {
                                    /*
                                    if (item.Status != System.Security.Cryptography.X509Certificates.X509ChainStatusFlags.RevocationStatusUnknown &&
                                        item.Status != System.Security.Cryptography.X509Certificates.X509ChainStatusFlags.OfflineRevocation)
                                        break;
                                        */
                                    if (item.Status == System.Security.Cryptography.X509Certificates.X509ChainStatusFlags.PartialChain)
                                    {
                                        PacketParser.Utils.Logger.Log("Ignoring SSL Chain Status validation error: " + item.Status.ToString() + " for " + item.ToString(), System.Diagnostics.EventLogEntryType.Information);
                                    }
                                    else if (item.Status != System.Security.Cryptography.X509Certificates.X509ChainStatusFlags.NoError)
                                    {
                                        PacketParser.Utils.Logger.Log("SSL Chain Status validation error: " + item.Status.ToString() + " for " + item.ToString(), System.Diagnostics.EventLogEntryType.Warning);
                                        return false;
                                    }
                                }
                            }
                ...
                {
                    PacketParser.Utils.Logger.Log("Error disabling SSL verification in Mono " + e.Message, System.Diagnostics.EventLogEntryType.Error);
                }
        ...
        private void AddMessages(List<PacketParser.Events.MessageEventArgs> mList)
        {
            ...
            foreach (PacketParser.Events.MessageEventArgs me in mList)
            {
                ...
                var kvp = new KeyValuePair<System.Collections.Specialized.NameValueCollection, byte[]>(me.Attributes, me.MessageEncoding.GetBytes(me.Message));
                int messageMaxLength = 10000;
                if (kvp.Value?.Length > messageMaxLength)
                {
                    byte[] newMessage = new byte[messageMaxLength];
                    Array.Copy(kvp.Value, newMessage, messageMaxLength);
                    kvp = new KeyValuePair<System.Collections.Specialized.NameValueCollection, byte[]>(kvp.Key, newMessage);
                }
                item.Tag = kvp;
                newItems.Add(item);
        ...
        */
        ...
        public void AddPcapFileToCasePanel(CaseFile caseFile, bool computeMd5)
        {
            ...
            else
            {
                ...
                if (computeMd5)
                {
                    caseFile.Md5 = NetresecShared.Md5SingletonHelper.Instance.GetMd5Sum(caseFile.FilePathAndName);
        ...
        private void stopButton_Click(object sender, EventArgs e)
        {
            ...
            if (packetHandlerWrapper.PcapWriter != null && packetHandlerWrapper.PcapWriter.Filename != null && casePanelFileListView.Items.ContainsKey(packetHandlerWrapper.PcapWriter.Filename))
                casePanelFileListView.Items[packetHandlerWrapper.PcapWriter.Filename].SubItems.Add(NetresecShared.Md5SingletonHelper.Instance.GetMd5Sum(packetHandlerWrapper.PcapWriter.Filename));
        }
...
2. Start NetworkMiner with administrator rights.");
        ...
        private void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (this.openPcapFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (!this.pcapFiles.Contains(filepath) && filepath.Length > 3)
                {
        ...
        void fileWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            PcapFileReader pcapReader = lp.PcapReader;
        ...
        private void clearGUIToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            this.openPcapFileDialog.FileName = String.Empty;
...
try {
    ...
    $pipeStream.Dispose();
        ...
        private void casePanelFileListView_MouseDown(object sender, MouseEventArgs e)
        {
            //we don't wanna do drag-and-drop on double clicks
            if (e.Clicks == 1 && e.Button == System.Windows.Forms.MouseButtons.Left)
            {
                ListViewItem fileItem = this.casePanelFileListView.GetItemAt(e.X, e.Y);
                if (fileItem != null && fileItem.Tag != null && fileItem.Tag is CaseFile caseFile)
                {
                    ...
                    if (System.IO.File.Exists(caseFile.FilePathAndName))
                    {
                        ...
                        {//Pokemon exception mode: catch 'em all
                            PacketParser.Utils.Logger.Log("Error on DoDragDrop of " + caseFile.FilePathAndName, System.Diagnostics.EventLogEntryType.Warning);
                        }
                    }
                }
            }
        ...
        private void fromOtherPcapFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (!this.pcapFiles.Contains(this.processor.file_selected_textbox.Text) && this.processor.file_selected_textbox.Text.Length > 3)
            {
                this.pcapFiles.Add(this.processor.file_selected_textbox.Text);
                this.processor.file_selected_textbox.Clear();
            }
            this.processor.file_selected_textbox.Text = string.Empty;
        }
